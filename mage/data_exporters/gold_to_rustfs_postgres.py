...
def export_gold_postgres(data, *args, **kwargs):
    ...
    _upload_df(
        client,
        bucket,
        f"demo_daily/dt={date_str}/{run_id}.parquet",
        data.get("gold_daily"),
    )
    _upload_df(
        client,
        bucket,
        f"demo_by_region/dt={date_str}/{run_id}.parquet",
        data.get("gold_region"),
    )
    _upload_df(
        client,
        bucket,
        f"demo_by_category/dt={date_str}/{run_id}.parquet",
        data.get("gold_category"),
    )
