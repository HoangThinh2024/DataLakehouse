...
def _insert(client: Client, table: str, df: pd.DataFrame, columns: list[str]) -> int:
    ...
    for col in df_insert.select_dtypes(include=["object"]).columns:
        if col in datetime_cols or col in date_cols:
            continue
        df_insert[col] = (
            df_insert[col].astype(str).replace({"None": None, "nan": None, "NaT": None})
        )
...
def _dedup_by_keys(df: pd.DataFrame, keys: list[str]) -> pd.DataFrame:
    ...
    sort_col = (
        "_db_processed_at"
        if "_db_processed_at" in df.columns
        else ("_silver_processed_at" if "_silver_processed_at" in df.columns else None)
    )
    if sort_col:
    ...
    if removed:
        print(
            f"[load_to_clickhouse] Pre-insert dedup ({present_keys}): removed {removed} duplicates"
        )
    return df
...
def load_clickhouse(data, *args, **kwargs):
    ...
    try:
        ...
        client.execute(
            f"INSERT INTO {db}.pipeline_runs ({', '.join(cols)}) VALUES",
            [
                {
                    "run_id": run_id,
