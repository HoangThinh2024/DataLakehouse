...
def export_data(data, *args, **kwargs):
    ...
    if not isinstance(data, dict):
        print(
            f"[load_excel_to_clickhouse] Skip run - invalid payload type: {type(data)}"
        )
        return {}
    if data.get("skip"):
        return {}
    ...
    if new_cols:
        ...
        for col in new_cols:
            try:
                client.execute(
                    f"ALTER TABLE {db}.{table_name} ADD COLUMN `{col}` Nullable(String)"
                )
        ...
        events.append(
            {
                ...
                "source_last_modified": dt.datetime.fromisoformat(
                    f["source_last_modified"]
                )
                if f.get("source_last_modified")
                else None,
                "status": "success",
                "row_count": len(df),
