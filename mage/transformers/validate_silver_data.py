    ...
    try:
        ...
        if "Trạng thái" in df.columns:
            ...
            suite.add_expectation(
                ExpectationConfiguration(
                    ...
                    kwargs={"column": "Trạng thái", "value_set": allowed_statuses},
                )
            )
                ...
                dq_results.append(
                    {
                        ...
                        if result["result"].get("unexpected_percent")
                        else 0.0,
                        "observed_value": str(
                            result["result"].get("observed_value", "")
                        ),
                        "element_count": float(
        ...
        if dq_results:
            try:
                ...
                print(
                    f"[validate_silver_data] Logged {len(dq_results)} DQ metrics to ClickHouse."
                )
        ...
        if not validation_result.success:
            ...
            print(
                "[validate_silver_data] PIPELINE CONTINUING (soft failure enabled for monitoring)"
            )
