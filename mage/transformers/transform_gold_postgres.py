...
def transform_gold_postgres(df, *args, **kwargs):
    ...
    gold_daily["avg_order_value"] = (
        gold_daily["total_revenue"] / gold_daily["order_count"]
    ).round(2)
    ...
    gold_region["avg_order_value"] = (
        gold_region["total_revenue"] / gold_region["order_count"]
    ).round(2)
    ...
    gold_category["avg_order_value"] = (
        gold_category["total_revenue"] / gold_category["order_count"]
    ).round(2)
    gold_category["_pipeline_run_id"] = run_id
    gold_category["_gold_processed_at"] = gold_ts

    print(
        f"[transform_gold_postgres] Daily rows: {len(gold_daily)}, Region rows: {len(gold_region)}, Category rows: {len(gold_category)}"
    )
