...
def run_dbt(*args, **kwargs):
    ...
    # Run dbt run
    cmd_run = ["dbt", "run", "--project-dir", project_dir, "--profiles-dir", profiles_dir]
    ...
    cmd_test = ["dbt", "test", "--project-dir", project_dir, "--profiles-dir", profiles_dir]
    res_test = subprocess.run(cmd_test)
    
    if res_test.returncode != 0:
        print(f"[dbt] WARNING: Some dbt tests failed with exit code {res_test.returncode}.")
    else:
