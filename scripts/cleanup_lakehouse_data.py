...
def main():
    ...
    reports_count = ch_client.execute("SELECT count() FROM analytics.project_reports")[
        0
    ][0]
    summary_count = ch_client.execute(
        "SELECT count() FROM analytics.fct_projects_summary"
    )[0][0]
