...
def fetch_rows() -> Tuple[List[str], List[Dict[str, Any]]]:
    ...
    if custom_db and custom_user and custom_password:
        ...
        source_db_name = get_env(
            "SOURCE_DB_NAME", get_env("POSTGRES_DB", "datalakehouse")
        )
        source_db_user = get_env(
            "SOURCE_DB_USER", get_env("POSTGRES_USER", "dlh_admin")
        )
        source_db_password = get_env(
            "SOURCE_DB_PASSWORD", get_env("POSTGRES_PASSWORD", "")
        )
    ...
    if not source_table:
        candidates = [
            name.strip()
            for name in (
                get_env("SOURCE_TABLE_CANDIDATES", "Demo,test_projects,sales_orders")
                or ""
            ).split(",")
            if name.strip()
        ]
...
def upload_to_rustfs(csv_text: str, metadata: Dict[str, Any]) -> Tuple[str, str]:
    ...
    object_key = (
        f"{rustfs_prefix}/dt={dt.date.today().isoformat()}/demo_{timestamp}.csv"
    )
    metadata_key = (
        f"{rustfs_prefix}/dt={dt.date.today().isoformat()}/demo_{timestamp}.json"
    )
...
def clickhouse_post(query: str, body: bytes | None = None) -> None:
    ...
    url = (
        f"{clickhouse_url}/?{urllib.parse.urlencode({'database': clickhouse_database})}"
    )
    payload = (
        query.encode("utf-8") if body is None else query.encode("utf-8") + b"\n" + body
    )
...
def load_into_clickhouse(columns: List[str], csv_text: str) -> None:
    ...
    column_definitions = ",\n  ".join(
        f"`{column}` Nullable(String)" for column in safe_columns
    )
...
def main() -> int:
    ...
    effective_db = custom_db or get_env(
        "SOURCE_DB_NAME", get_env("POSTGRES_DB", "datalakehouse")
    )
    ...
    print(
        f"Extracted {len(rows)} rows from source table {effective_schema}.{source_table}."
    )
    print(f"Uploaded CSV to RustFS: {rustfs_key}")
    print(f"Uploaded metadata JSON to RustFS: {metadata_key}")
    print(
        f"Loaded raw rows into ClickHouse: {get_env('CLICKHOUSE_DB', 'analytics')}.{get_env('CLICKHOUSE_TABLE', 'demo_raw')}"
    )
    return 0
