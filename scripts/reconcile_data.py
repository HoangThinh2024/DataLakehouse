...
RUSTFS_ENDPOINT = os.getenv("RUSTFS_ENDPOINT_URL", "http://localhost:29100").replace(
    "dlh-rustfs:9000", "localhost:29100"
)
...
def get_missing_files():
    ...
    try:
        ...
        ch = get_ch_client()
        rows = ch.execute(
            "SELECT source_key, etag FROM excel_upload_events WHERE status = 'success'"
        )
        ...
        missing = [
            ...
            if k not in processed or processed.get(k) != v
        ]
        if missing:
...
def run_once():
    ...
    if not is_synced:
        logger.warning(
            f"Data out of sync! ClickHouse: {actual}, Expected: {expected}. Repairing..."
        )
        ...
        ch = get_ch_client()
        ch.execute(
            "DELETE FROM excel_upload_events WHERE source_key = (SELECT source_key FROM excel_upload_events ORDER BY processed_at DESC LIMIT 1)"
        )
