...
def _s3_client(endpoint_url: str | None = None):
    ...
    return boto3.client(
        ...
        or os.getenv("RUSTFS_ENDPOINT_URL", "http://dlh-rustfs:9000"),
...
def check_rusfs_layers(results: dict) -> bool:
    ...
    for layer_name, bucket in layers.items():
        ...
        try:
            ...
            if not results.get("json_mode"):
                print(
                    f"✓ {layer_name.upper()} bucket exists: {bucket} ({count} objects)"
                )
...
def check_data_lineage(results: dict) -> bool:
    ...
    checks = [
        (
            "bronze",
            os.getenv("RUSTFS_BRONZE_BUCKET", "bronze"),
            os.getenv("RUSTFS_BRONZE_PREFIX", "demo"),
        ),
        (
            "silver",
            os.getenv("RUSTFS_SILVER_BUCKET", "silver"),
            os.getenv("RUSTFS_SILVER_PREFIX", "demo"),
        ),
        ("gold", os.getenv("RUSTFS_GOLD_BUCKET", "gold"), ""),
    ]
...
def check_clickhouse_architecture(results: dict) -> bool:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=min(len(probes), 10)) as executor:
            futures = {
                executor.submit(_probe_ch_endpoint, h, p): (h, p) for h, p in probes
            }
            for future in as_completed(futures):
                res = future.result()
                if res:
        ...
        for table in expected:
            ...
            if table in table_names:
                ...
                table_res["rows"] = ch_client.execute(f"SELECT count() FROM {table}")[
                    0
                ][0]
                if not results.get("json_mode"):
                ...
                if table == "silver_demo":
                    ...
                    if (
                        "ENGINE = S3" not in create_stmt
                        and "ENGINE = DeltaLake" not in create_stmt
                        and "ENGINE = ReplacingMergeTree" not in create_stmt
                    ):
