...
def _probe_s3(endpoint):
    try:
        ...
        if not is_port_open(parsed.hostname, parsed.port or 80):
            return None
        client = boto3.client(
            "s3",
...
RUSTFS_INTERNAL_ENDPOINT = os.getenv(
    "RUSTFS_INTERNAL_ENDPOINT", "http://dlh-rustfs:9000"
)
...
def get_effective_s3_endpoint():
    ...
    if _cached_s3_endpoint:
        return _cached_s3_endpoint

    endpoints = [
        os.getenv("RUSTFS_S3_ENDPOINT"),
...
def get_effective_ch_endpoint():
    ...
    if _cached_ch_endpoint:
        return _cached_ch_endpoint
...
def get_s3_client():
    return boto3.client(
        "s3",
        endpoint_url=get_effective_s3_endpoint(),
...
def backup_clickhouse():
    ...
    try:
        ...
        with Client(
            ...
            logger.info(
                f"✓ Backup successful: s3://{BACKUP_BUCKET}/clickhouse/{date_str}/"
            )
    ...
    try:
        ...
        for bucket in buckets:
            ...
            try:
                ...
                if bucket_deleted > 0:
                    logger.info(
                        f"  → Deleted {bucket_deleted} expired objects from {bucket}"
                    )
